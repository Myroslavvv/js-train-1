...
function isPositiveNumber(num) {
  ...
  let isPositiveNumber = (num <= 0) ? (false) : true;
...
function isEmptyString(str) {
  ...
  let isEmptyString = (str.length) ? (false) : true;
...
function getVariableType(variable) {
  ...
  let type = (typeof variable === 'number') ? 'Це число' : (typeof variable === 'string') ? 'Це рядок' : (typeof variable === 'boolean') ? 'Це булеве значення' : 'Це інший тип';
  return type;
}
